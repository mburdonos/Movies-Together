...
def login_by_user(
    ...
    device, _ = device_service.get_or_create_allowed_device(user_id, fingerprint)
    device_id = device["id"]
...
def logout(
    ...
    **kwargs
    ...
    device = device_service.get(user_id=_user_id, fingerprint=request.user_agent.string)
...
def verify_token(
    ...
    required_permissions = schemas.VerifyTokenRequest(**request.get_json()).permissions
