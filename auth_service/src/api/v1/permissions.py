...
permission_routes = Blueprint("permission_routes", __name__, url_prefix="/api/v1")
...
def get_permission(
    _user_id: int, service: PermissionService = get_permission_service(uow()), **kwargs
) -> tuple[list[dict], int]:
    return [
...
@permission_routes.route("/permissions/<int:permission_id>/", methods=["DELETE"])
@exception_wrapper("permission")
@rate_limit()
